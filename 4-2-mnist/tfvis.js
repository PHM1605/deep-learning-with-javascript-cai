...
export const render = {
  ...
  // config: {width:450, height:320, xLabel:"FPR", yLabel:"TPR"...}
  linechart: (container, dataConfig, config) => {
    // Plotly addtraces
    const traceList = dataConfig.series.map((seriesName, idx) => (
      {
        x: dataConfig.values[idx].map(onePoint => onePoint.x),
        y: dataConfig.values[idx].map(onePoint => onePoint.y),
        name: seriesName
      }
    ));
    Plotly.newPlot(container, traceList, {

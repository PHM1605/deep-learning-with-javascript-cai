...
const dataTrace100Epochs = {
  x: [0, 2], y: [0, 0.01],
  name: 'model after 100 epochs',
  mode: 'lines',
  line: {color: 'red', width:3, dash:'long-dash'}
}
...
Plotly.newPlot(
  'dataSpace',
  [dataTraceTrain, dataTraceTest, dataTrace10Epochs, dataTrace20Epochs, dataTrace100Epochs, dataTrace200Epochs],
...
// Plot params
const kTrace = {x:[], y:[], name:"kernel"}
const bTrace = {x:[], y:[], name:"bias"}
function plotParams(epoch, k, b) {
  kTrace.x.push(epoch);
  kTrace.y.push(k);
  bTrace.x.push(epoch);
  bTrace.y.push(b);
  Plotly.newPlot('paramsPlot', [kTrace, bTrace], {width:400, xaxis:{title:"epoch #"}})
}
...
const trace0 = {x:[], y:[], name:"minimum by eye", mode:"markers", type:"scatter", marker:{symbol:"circle", size:10, color:"white"}}
const trace20 = {x:[], y:[], name:"minimum by eye", mode:"markers", type:"scatter", marker:{symbol:"circle", size:10, color:"white"}}
const trace100 = {x:[], y:[], name:"minimum by eye", mode:"markers", type:"scatter", marker:{symbol:"circle", size:10, color:"white"}}
const trace200 = {x:[], y:[], name:"minimum by eye", mode:"markers", type:"scatter", marker:{symbol:"circle", size:10, color:"white"}}

function recordTrajectory(k, b, loss) {
  trajectory.x.push(k);
  trajectory.y.push(b);
  contourAnnotationFont = {family:"sans serif", size:18, color:"#ffffff"};
  Plotly.newPlot('lossSurface2dWithTraj', [landscape, trajectory, trace0, trace20, trace100, trace200], {
    height:500, width:500, title:"Loss surface", xaxis:{title:"kernel"}, yaxis:{title:"bias"}, showlegend:false,
    annotations:[
      {x:0, y:0, xref:"x", yref:"y", text:"Start", showarrow:true, arrowhead:0, arrowcolor:"#ffffff", font:contourAnnotationFont, ax:0, ay:-40},
      {x:0.028, y:0.01, xref:"x", yref:"y", text:"Epoch 20", showarrow:true, arrowhead:0, arrowcolor:"#ffffff", font:contourAnnotationFont, ax:0, ay:40},
      {x:0.076, y:0.039, xref:"x", yref:"y", text:"Epoch 100", showarrow:true, arrowhead:0, arrowcolor:"#ffffff", font:contourAnnotationFont, textposition:"right", ax:60, ay:0},
      {x:0.072, y:0.065, xref:"x", yref:"y", text:"Epoch 200", showarrow:true, arrowhead:0, arrowcolor:"#ffffff", font:contourAnnotationFont, textposition:"right", ax:60, ay:0}
    ]
  })
}

// move the white dots from "Start" to "Epoch 20", "Epoch 100" etc. locations
